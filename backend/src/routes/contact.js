...
router.post('/send', async (req, res) => {
    ...
    try {
        ...
        if (!verifyResponse.data.success) {
            console.warn("Turnstile Verification Failed:", verifyResponse.data);
      ...
      `
        ...
        if (process.env.SMTP_USER && process.env.SMTP_SERVER) {
            ...
            console.log(`Email enviado con éxito de ${email}`);
        } else {
            console.log("---- SIMULACIÓN DE ENVÍO DE EMAIL (Sin Credenciales SMTP) ----");
